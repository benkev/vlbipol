...
The input parameter of \verb@find_tail_bounds(ni)@ is an array containing a distribution of observed frequencies. The \verb@find_tail_bounds(ni)@ function outputs the indices at which the sparse tails are to be cut, \verb@(l_idx, r_idx)@, where
\begin{itemize}
  \item \verb@l_idx@ points AT the first ni element greater than thr, and \\
  \item \verb@r_idx@ points AFTER the last ni element greater than thr. \\
\end{itemize}

So, \verb@ni[l_idx : r_idx]@ is the subarray whose elements are > thr. \\
The function \verb@ni =  group_tails(ni,  (l_idx, r_idx))@ sums up the sparse tail values and adds them to the border values on the either side. Using the tail indices \verb@(l_idx, r_idx)@ one can make the tail grouping on the same size array of expected (theoretical) frequencies, calculated from a distribution to which \verb@ni@ has been fit.

\subsection{Plot Parameter Temporal Variations}

The script \verb@plot_time_par_2187_2187.py@ creates plots for MBD, SBD, and SNR time variations. The data are read from the dictionaries \verb@idx2187l.pkl@ and \verb@idx2187cI.pkl@.\\
