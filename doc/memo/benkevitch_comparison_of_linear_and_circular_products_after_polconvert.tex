...
To compare the observed values of the parameters LinI and CirI with their unbiased residual RMS error, we write it in the format $\overline{\mathrm{PRM}} \pm \mathrm{RMSE}$ or $\overline{\mathrm{PRM}} \pm \sigma$. The relative error between the unbiased LinI and CirI can be expressed as
...
\begin{table}[ht!]
  \begin{center}
    ...
    \begin{tabular}{l|r|r|c|r|r}
      BL & $\overline{\rm{SNR}}$ & $\overline{\rm{MBD}}\pm\sigma\, (\mu s)$ & \text{relerr (\%)} & $\overline{\rm{Bias}}\, (\mu s)$ & \multicolumn{1}{c}{$r_{corr}$} \\
      \hline
...
\begin{table}[ht!]
  \begin{center}
    ...
    \begin{tabular}{l|r|r|c|r|r}
      BL & $\overline{\rm{SNR}}$ & $\overline{\rm{SBD}}\pm\sigma\, (\mu s)$ & \text{relerr (\%)} & $\overline{\rm{Bias}}\, (\mu s)$ & \multicolumn{1}{c}{$r_{corr}$} \\
      \hline
...
The script \verb@plot_time_var.py@ creates plots for MBD, SBD, and SNR time variations. \\
Usage: \\
...
The plots generated are shown in Figures~\ref{mbd_lin_and_cir} to~\ref{snr_lin_minus_cir}.  The data are read from the dictionaries \verb@idx3819l.pkl@ and \verb@idx3819cI.pkl@.
