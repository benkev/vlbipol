...
\indent Lin\_I[$k$] = Lin\_I[$k$] - mean(Lin\_I[$k$]) \\
\indent Cir\_I[$k$] = Cir\_I[$k$] - mean(Cir\_I[$k$]) \\

\noindent In order to compare Lin\_I[$k$] with Cir\_I[$k$], the residual (res[$k$]) is computed as their difference: \\

%\indent res[$k$] = Lin\_I[$k$] - Cir\_I[$k$] \\
...
While in the previous Section MBD, SBD, and SNR pseudo-Stokes $I$ parameter data were treated as functions of time, here we do their statistical study. In order to estimate the errors introduced by PolConvert, histograms of the parameter residuals (after the mean subtraction, see Eqs.~\eqref) are plotted. Below are the histograms for all of the baselines: \\
