...
def plot_closure_legend(ax_col, cols, par, fs=12):
    '''
    ...
    Inputs:
        ...
        cols:   Dict of colors, cols[triangle] --> 4-array, each array is a row
                in pyplot.cm(numbers 0 to 1)
                For example:
                  acols = cm.nipy_spectral(1 - np.linspace(0, 1, ntri))
    ...
    for i in range(qntri):
        ...
        k = i                  # Index into trians[k]
        tr = trians[k]         # Triangle
        rect = patches.Rectangle((0, y), .95, .95, facecolor=cols[tr])
        ...
        k = i + qntri
        tr = trians[k]         # Triangle
        rect = patches.Rectangle((2, y), .95, .95, facecolor=cols[tr])
        ax_col.add_patch(rect)
        ...
        k = i + 2*qntri
        tr = trians[k]         # Triangle
        rect = patches.Rectangle((4, y), .95, .95, facecolor=cols[tr])
        ax_col.add_patch(rect)
        ...
        k = i + 3*qntri
        tr = trians[k]         # Triangle
        rect = patches.Rectangle((6, y), .95, .95, facecolor=cols[tr])
        ax_col.add_patch(rect)
