...
#
# Plot histograms of SBD differences for the baselines including station "sta"
#
...
r_corr = sum(sbd0_l*sbd0_c)/np.sqrt(sum(sbd0_l**2)*sum(sbd0_c**2))
    ...
    # tim = tim - tim[0]
    # # 
    # sbd_l_us = np.array(idx3819l_1[bl]['I']['sbdelay'])[istart:]
    # sbd_c_us = np.array(idx3819c_1[bl]['I']['sbdelay'])[istart:]

    # sbd_l = sbd_l_us*1e6           # Convert us to ps
    # sbd_c = sbd_c_us*1e6           # Convert us to ps
    # #sbd_a = (sbd_l + sbd_c)/2      # Average of the lin and cir curves
    
    # sbd_all_l = np.append(sbd_all_l, sbd_l)
    # sbd_all_c = np.append(sbd_all_c, sbd_c)
    ...
    # print("'%s': abs(sbd0_l).mean() = %.2f (ps),\t "
    #       "abs(sbd0_c).mean() = %.2f (ps)"% \
    #       (bl, abs(sbd0_l).mean(), abs(sbd0_c).mean()))
    ...
    # rmse_sbd[ibl] = np.sqrt(np.sum(dsbd_bl**2)/npt)
    # r_corr_sbd[ibl] = sum(sbd0_l*sbd0_c)/np.sqrt(sum(sbd0_l**2)*sum(sbd0_c**2))
